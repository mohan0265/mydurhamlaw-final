...
-- supabase_migration_premier_lounge_final.sql
-- Consolidated, idempotent migration for Premier Student Lounge.
-- Run this on new environments. Safe to re-run.
-- It creates tables, indexes, RLS, storage bucket, realtime, and RPCs.
...
for update using (auth.uid() is not null and (author_id = auth.uid() or public.is_service_role()));

drop policy if exists "posts_delete" on public.lounge_posts;
create policy "posts_delete" on public.lounge_posts
for delete using (auth.uid() is not null and (author_id = auth.uid() or public.is_service_role()));
...
-- Realtime: add tables to publication; ignore duplicates
do $$ begin
  begin execute 'alter publication supabase_realtime add table public.lounge_posts'; exception when others then null; end;
  begin execute 'alter publication supabase_realtime add table public.lounge_sparks'; exception when others then null; end;
  begin execute 'alter publication supabase_realtime add table public.lounge_reactions'; exception when others then null; end;
end $$;
...
returns void language plpgsql security definer as $$
begin
  if not public.is_service_role() then raise exception 'admin only'; end if;
...
begin
  ...
  insert into public.moderation_actions(target_type, target_id, action, actor_id, reason)
  values ('user', p_user_id, 'shadow_mute', 'service');
end $$;
...
begin
  ...
  insert into public.moderation_actions(target_type, target_id, action, actor_id, reason)
  values ('user', p_user_id, 'unshadow_mute', 'service');
end $$;
