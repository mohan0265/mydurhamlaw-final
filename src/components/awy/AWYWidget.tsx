...
// Bottom-right default: above Durmah mic
function computeBottomRight(): Position {
  if (typeof window === "undefined") return { x: 16, y: 16 };
  const w = window.innerWidth;
  const h = window.innerHeight;
  const widgetWidth = 64; // collapsed width
  const safeRight = 88;   // room for mic
  const safeBottom = 180; // above mic
  return {
    x: Math.max(16, w - widgetWidth - safeRight),
    y: Math.max(16, h - safeBottom),
  };
}
...
export default function AWYWidget() {
  ...
  // Set starting position on the client (SSR-safe)
  useEffect(() => {
    const pos = getLastPosition();
    setPosition(pos);
  }, []);

  // Feature flag + auth (do this AFTER hooks run)
  if (process.env.NEXT_PUBLIC_FEATURE_AWY !== "1") return null;
  if (!userId) return null;

  // Presence transition → toast when someone comes online
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <motion.div
                            className={`w-9 h-9 rounded-full ring-2 ${ringClass(
                              status
