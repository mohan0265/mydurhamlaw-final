...
export default function AssignmentWorkflow({
  ...
  // Jump Logic State
  const [showJumpConfirm, setShowJumpConfirm] = useState<{
    target: number;
    name: string;
  } | null>(null);
  ...
  const insertIntoDraft = (payload: InsertPayload) => {
    ...
    if (payload.addPrefix) {
      const prefixLabel =
        payload.label ||
        (payload.source === "durmah" ? "Durmah suggestion" : "Stage notes");
  ...
  const loadProgress = async () => {
    ...
    try {
      ...
      console.log("[RESUME DEBUG] Progress query result:", {
        progress,
        progressError,
      ...
      if (progressError) {
        ...
        setUploadMode(false);
        toast.success(
          `Resuming at Stage ${progress.current_stage}: ${stages[progress.current_stage - 1]?.name}`,
        );
      } else {
        console.log(
          "[RESUME DEBUG] No saved progress found or current_stage is 0",
        );
      }
  ...
  const saveProgress = async (stage: number, data: any) => {
    ...
    try {
      ...
      await supabase.from("assignment_stages").upsert({
          ...
          {!uploadMode && (
            <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-white/20">
              ...
              {stages.map((stage, idx) => (
                <div
                  key={stage.num}
                  className="flex flex-col items-center gap-1 shrink-0 px-2 group cursor-pointer relative"
                  onClick={() => {
                    ...
                    if (
                      ...
                      stageData[`stage_${stage.num}_complete`]
                    ) {
                    ...
                    <button
                      className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-all z-10 ${
                        currentStage === stage.num
                          ? "bg-white text-violet-600 ring-4 ring-white/30 scale-110"
                          : stage.num < currentStage
        ...
        <div className="absolute inset-0 z-[80] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white rounded-2xl p-6 shadow-2xl max-w-md w-full border border-gray-100">
            ...
            <h3 className="text-xl font-bold text-gray-900 text-center mb-2">
              Jump to {showJumpConfirm.name}?
            </h3>
            <p className="text-gray-600 text-center text-sm mb-6">
              You're skipping ahead. This is great if you've already done work
              outside the app.
              <br />
              <br />
              We'll let you paste your work, but Durmah might have less context
              to help you.
            </p>
