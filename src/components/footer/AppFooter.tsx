...
export const AppFooter: React.FC<AppFooterProps> = ({ isAuthed }) => {
  ...
  const seoLinks = [
    ...
    {
      label: "Legal Assignment Assistance",
        ...
        <li key={idx}>
          {link.disabled ? (
            <span className="text-gray-500 cursor-not-allowed flex items-center text-xs">
              {link.icon || null}
              {link.label}
              <span className="ml-2 text-[10px] uppercase bg-gray-800 text-gray-400 px-1.5 py-0.5 rounded">
                Soon
              </span>
            </span>
          ) : (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          ...
          <div>
            ...
            {isAuthed
              ? renderLinks(studentStudyLinks)
              : renderLinks(publicLearnLinks)}
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              ...
              <li className="pt-2">
                <Link
                  href="/admin/login"
                  className="text-gray-500 hover:text-white transition-colors text-xs flex items-center gap-2 mt-2 pt-2 border-t border-gray-800"
                >
        ...
        {!isAuthed && (
          <div className="border-t border-gray-700 pt-4 mb-6">
            <h5 className="text-[10px] font-semibold text-gray-500 mb-2 uppercase tracking-wider">
              Popular Topics
            </h5>
        ...
        <div className="border-t border-gray-700 pt-6 flex flex-col md:flex-row justify-between items-center text-xs">
          ...
          <div className="flex items-center space-x-4">
            {!isAuthed && (
              <Link
                href="/signup"
                className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-full transition-all text-xs font-medium border border-white/10"
              >
                Start Free Trial
              </Link>
            )}
            <Link
              href="/admin/login"
              className="text-gray-600 hover:text-white transition-colors text-[10px]"
            >
              Admin
            </Link>
            <a
