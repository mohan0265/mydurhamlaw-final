...
export default function ResponsiveSidebar({
  ...
  // Desktop collapse state (default collapsed)
  ...
  // Load collapsed state (client only)
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      const saved = window.localStorage.getItem("mdl:navCollapsed");
  ...
  useEffect(() => {
    const handleRouteChangeStart = () => setIsMobileOpen(false);
  ...
  // Onboarding badges intentionally disabled
  const shouldShowOnboardingBadge = () => false;
  const getOnboardingBadgeText = () => null as any;
  ...
  const isActive = (href: string) => {
    if (href.includes("#")) return router.pathname === href.split("#")[0];
