        ...
        {navigationItems.map((item, index) => {
          ...
          return (
            <Link key={index} href={item.href}>
              ...
              `}>
                <IconComponent className={`w-5 h-5 ${active ? 'scale-110' : 'group-hover:scale-105'} transition-transform duration-200`} />
                {!isCollapsed && (
          ...
          return (
            <Link key={`personalization-collapsed-${index}`} href={item.href}>
              ...
              `}>
                <IconComponent className={`w-5 h-5 ${active ? 'scale-110' : 'group-hover:scale-105'} transition-transform duration-200`} />
                {item.needsBadge && shouldShowOnboardingBadge() && (
                  <div className={`absolute -top-1 -right-1 w-3 h-3 rounded-full border-2 border-white ${onboardingStatus === 'partial' && onboardingProgress >= 50 ? 'bg-orange-400' : 'bg-red-400'}`}></div>
                )}
