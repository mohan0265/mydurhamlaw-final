...
export function QuickActionsWidget() {
  ...
  const [hasAssignmentDraft, setHasAssignmentDraft] = useState(false);
  const [hasLectureDraft, setHasLectureDraft] = useState(false);

  useEffect(() => {
    setHasAssignmentDraft(!!localStorage.getItem("caseway_assignment_draft"));
    setHasLectureDraft(!!localStorage.getItem("caseway_lecture_draft"));
  }, []);
          ...
          {quickActions.map((action: any, index) => (
            ...
            >
              <div className="flex flex-col items-center text-center space-y-2">
                ...
                <div>
                  <h4 className="font-semibold text-sm flex items-center justify-center gap-1.5">
                    {action.title}
                    {action.hasDraft && (
                      <span
                        className="flex h-2 w-2 rounded-full bg-red-400 animate-pulse ring-2 ring-white"
                        title="Active Draft"
                      />
                    )}
                  </h4>
