...
export const ModuleGroup: React.FC<ModuleGroupProps> = ({
  ...
  const getProgressStatus = () => {
    ...
    if (progress.percentage < 25)
      return { text: "Getting Started", color: "text-blue-600" };
    if (progress.percentage < 50)
      return { text: "In Progress", color: "text-orange-600" };
    if (progress.percentage < 75)
      return { text: "Well Underway", color: "text-yellow-600" };
    if (progress.percentage < 100)
      return { text: "Nearly Complete", color: "text-green-600" };
  ...
  return (
    <Card
      className={`${colors.border} border-2 hover:shadow-lg transition-all duration-300`}
    >
      {/* Module Header */}
      <div
                ...
                {upcomingDeadlines.map((deadline, index) => (
                  ...
                  >
                    ...
                    <span className="text-xs text-orange-700">
                      {new Date(
                        (deadline as any).due_at || (deadline as any).date,
