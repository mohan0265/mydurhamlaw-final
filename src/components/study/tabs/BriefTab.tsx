...
export default function BriefTab({ assignment, onUpdate }: BriefTabProps) {
  ...
  useEffect(() => {
    ...
    else if (assignment.brief_rich &&  typeof assignment.brief_rich === 'object') b = JSON.stringify(assignment.brief_rich);
    ...
    <div className="h-full flex flex-col space-y-4 animate-in fade-in duration-300">
       <div className="flex flex-wrap items-center gap-4 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-xl border border-gray-200 dark:border-gray-800 transition-colors">
         <div className="flex-1 min-w-[140px]">
            <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Word Target</label>
            <input 
              type="number" 
         ...
         <div className="flex-1 min-w-[140px]">
            <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Weightage</label>
            <input 
              type="text" 
              className="w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-violet-500 outline-none text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
              value={weightage}
       ...
       <div className="flex-1 flex flex-col">
          <div className="flex justify-between items-center mb-2">
             <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Detailed Brief / Instructions</label>
             <button
               onClick={() => {
                  ...
                  const wordMatch = text.match(/(?:word count|length|limit)[:\s]+(\d{1,5})/i);
                  if (wordMatch) {
                  ...
                  if (Object.keys(updates).length > 0) {
                      ...
                      toast('No standard patterns found. Try pasting the full Blackboard specification.', { icon: 'ℹ️' });
                  }
               }}
          ...
          />
          <p className="text-xs text-gray-400 mt-2 text-right">Supports Markdown (coming soon)</p>
       </div>
    </div>
         <div className="flex-1 min-w-[140px]">
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Word Target</label>
            <input 
              type="number" 
              className="w-full bg-white border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-violet-500 outline-none"
              value={wordCount}
              onChange={(e) => setWordCount(e.target.value ? Number(e.target.value) : '')}
              placeholder="e.g. 2000"
            />
         ...
         <div className="flex-1 min-w-[140px]">
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Weightage</label>
            <input 
              type="text" 
              className="w-full bg-white border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-violet-500 outline-none"
              value={weightage}
       ...
       <div className="flex-1 flex flex-col">
          <div className="flex justify-between items-center mb-2">
             <label className="block text-xs font-bold text-gray-500 uppercase">Detailed Brief / Instructions</label>
             <button
               onClick={() => {
                  // Simple Blackboard Parser
                  const text = brief;
               ...
               className="text-xs font-bold text-violet-600 hover:text-violet-800 bg-violet-50 px-2 py-1 rounded transition"
             >
               ✨ Auto-Extract Details
             </button>
          </div>
          <textarea 
            className="flex-1 w-full border border-gray-200 rounded-xl p-4 text-sm leading-relaxed focus:ring-2 focus:ring-violet-500 focus:border-transparent outline-none resize-none font-mono text-gray-700"
            value={brief}
            onChange={(e) => setBrief(e.target.value)}
            placeholder="Paste your module handbook instructions, essay questions, or Blackboard brief here..."
          />
