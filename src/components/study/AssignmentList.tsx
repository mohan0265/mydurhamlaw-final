...
export default function AssignmentList({ assignments, selectedId, onSelect, onNew }: AssignmentListProps) {
  ...
  const filteredAssignments = useMemo(() => {
    return assignments.filter(a => {
      const matchesFilter = 
        filter === 'all' ? true :
        filter === 'completed' ? a.status === 'completed' || a.status === 'submitted' :
          ...
          filteredAssignments.map((assignment) => (
            ...
            >
              <div className="flex justify-between items-start mb-1">
                <span className={`text-[10px] px-2 py-0.5 rounded-full border ${getStatusColor(assignment.status)}`}>
                  {assignment.status.replace('_', ' ')}
                </span>
                <span className={`text-xs flex items-center gap-1 ${
                  new Date(assignment.due_date) < new Date() ? 'text-red-500 font-medium' : 'text-gray-500'
                }`}>
                  <Clock size={12} />
              ...
              {assignment.module_name && (
                <p className="text-xs text-gray-500 line-clamp-1">
                  {assignment.module_code && <span className="font-mono text-[10px] mr-1 opacity-75">{assignment.module_code}</span>}
                  {assignment.module_name}
                </p>
              )}
            </button>
