...
export function useDurmahChat({
  ...
  useEffect(() => {
    ...
    if (scope === "global") {
      ...
      console.warn(
        "[useDurmahChat] Scope requires ID but none provided. Falling back to global.",
      );
      cId = uuidv5(`global-${user.id}`, DURMAH_NAMESPACE);
    }
  ...
  const fetchMessages = useCallback(
    async (cId: string) => {
      console.log(
        "[useDurmahChat] Fetching messages for Conversation ID:",
        cId,
      );
      if (!cId || !supabase) {
        console.warn(
          "[useDurmahChat] Cannot fetch: missing cId or supabase client",
        );
        return;
      }
      ...
      try {
        ...
        if (scope === "assignment") {
          ...
          if (data) {
            console.log(
              `[useDurmahChat] Fetched ${data.length} messages from assignment_session_messages`,
            );
          ...
          if (data) {
            console.log(
              `[useDurmahChat] Fetched ${data.length} messages from exam_messages`,
            );
            const deduped: any[] = [];
            data.forEach((m: any) => {
              const last = deduped[deduped.length - 1];
          ...
          if (data) {
            console.log(
              `[useDurmahChat] Fetched ${data.length} messages from database`,
            );
            setMessages(
              data.map((m: any) => ({
                id: m.id,
  ...
  const sendMessage = useCallback(
    async (content: string, modality: "text" | "voice" = "text") => {
      ...
      if (
        lastSendRef.current &&
        lastSendRef.current.text === normalized &&
        nowTs - lastSendRef.current.ts < 1500
      ) {
        console.warn(
          "[useDurmahChat] sendMessage: Blocking rapid duplicate send",
        );
        return;
      }
      ...
      if (!targetId && user) {
        if (scope === "assignment" && assignmentId) {
          targetId = uuidv5(
            `assignment-${user.id}-${assignmentId}`,
            DURMAH_NAMESPACE,
          );
      ...
      try {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === tempId ? { ...m, id: data.userMsgId || m.id } : m,
          ),
        );
  ...
  const toggleSaveMetadata = useCallback(
    async (
      msgId: string,
      currentVisibility: string | undefined,
      silent = false,
    ): Promise<boolean> => {
      if (!supabase) {
        console.error(
          "[useDurmahChat] Cannot toggle save: supabase client not available",
        );
      ...
      const newVisibility =
        currentVisibility === "saved" ? "ephemeral" : "saved";
      const newSavedAt =
        newVisibility === "saved" ? new Date().toISOString() : null;

      // Optimistic update
      setMessages((prev) =>
        prev.map((m) =>
          m.id === msgId
            ? { ...m, visibility: newVisibility as any, saved_at: newSavedAt }
            : m,
        ),
      );
      ...
      try {
          ...
          setMessages((prev) =>
            prev.map((m) =>
              m.id === msgId
                ? {
                    ...m,
                    visibility: currentVisibility as any,
                    saved_at: currentVisibility === "saved" ? m.saved_at : null,
                  }
                : m,
            ),
          );
        ...
        if (!silent) {
          toast.success(
            newVisibility === "saved" ? "Message saved" : "Message unsaved",
          );
        }
        return true;
      } catch (err) {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === msgId ? { ...m, visibility: currentVisibility as any } : m,
          ),
        );
        if (!silent) toast.error("Failed to update message");
        return false;
  ...
  const logMessage = useCallback(
    async (
      ...
      if (!conversationId || !user || !supabase) {
        console.warn(
          "[useDurmahChat] Cannot log message: missing required context",
        );
        return;
      }
  ...
  const deleteMessages = useCallback(
    async (ids: string[]) => {
      if (!ids.length || !supabase) {
        console.warn(
          "[useDurmahChat] Cannot delete: no IDs or supabase client missing",
        );
        return;
      }
  ...
  const clearUnsaved = useCallback(async () => {
    ...
    const unsavedMessages = messages.filter(
      (m) => !(m.visibility === "saved" || !!m.saved_at),
    );
    ...
    setMessages((prev) =>
      prev.filter((m) => m.visibility === "saved" || !!m.saved_at),
    );
  ...
  const createSession = useCallback(
    async (sessionId: string, title?: string) => {
      if (!user || !supabase) {
        console.warn(
          "[useDurmahChat] Cannot create session: missing user or supabase",
        );
        return false;
      }

      try {
        const { error } = await supabase.from("durmah_sessions").insert({
          id: sessionId,
          user_id: user.id,
          source,
          scope,
  ...
  const saveSession = useCallback(
    async (sessionId?: string, customTitle?: string) => {
      ...
      if (!targetSessionId || !user || !supabase) {
        console.warn(
          "[useDurmahChat] Cannot save session: missing required data",
        );
        return false;
      }
  ...
  const discardSession = useCallback(
    async (sessionId?: string) => {
      ...
      if (!targetSessionId || !supabase) {
        console.warn(
          "[useDurmahChat] Cannot discard session: missing required data",
        );
        return false;
      }
      ...
      try {
        if (scope === "assignment") {
          ...
          if (messagesError) {
            console.error(
              "[useDurmahChat] Delete assignment messages error:",
              messagesError,
            );
          ...
          if (sessionError) {
            console.error(
              "[useDurmahChat] Delete assignment session error:",
              sessionError,
            );
          }

          console.log(
            `[useDurmahChat] Assignment session ${targetSessionId} cleared successfully`,
          );
        } else if (scope === "exam") {
          const { error: messagesError } = await supabase
            .from("exam_messages")
          ...
          if (messagesError) {
            console.error(
              "[useDurmahChat] Delete exam messages error:",
              messagesError,
            );
            toast.error("Failed to clear messages");
            return false;
          ...
          if (sessionError) {
            console.error(
              "[useDurmahChat] Delete exam session error:",
              sessionError,
            );
          }

          console.log(
            `[useDurmahChat] Exam session ${targetSessionId} cleared successfully`,
          );
        } else {
          // Legacy/Global behavior
          ...
          if (messagesError) {
            console.error(
              "[useDurmahChat] Delete messages error:",
              messagesError,
            );
            toast.error("Failed to clear messages");
            return false;
          ...
          if (sessionError) {
            console.error(
              "[useDurmahChat] Delete session error:",
              sessionError,
            );
          }

          console.log(
            `[useDurmahChat] Session ${targetSessionId} cleared successfully`,
          );
        }
