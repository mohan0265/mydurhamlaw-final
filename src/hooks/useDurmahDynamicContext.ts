...
export type DurmahContextExtra = {
  publicName: string;
...
export function useDurmahDynamicContext() {
  ...
  const [identity, setIdentity] = useState<DurmahContextExtra>({
    publicName: "Student",
  });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch user identity for Durmah
        let identityData: DurmahContextExtra = { publicName: "Student" };
        try {
          const { data: profile } = await supabase
            .from("profiles")
            .select(
              "display_name, preferred_name, name_pronunciation, privacy_mask_name, role",
            )
            .eq("id", user.id)
            .single();

          if (profile) {
            const isDemo = profile.privacy_mask_name || profile.role === "demo";
            identityData.publicName = isDemo
              ? "Demo Student"
              : profile.preferred_name || profile.display_name || "Student";
            if (!isDemo && profile.name_pronunciation) {
              identityData.pronunciationHint = profile.name_pronunciation;
            }
            setIdentity(identityData);
          }
        } catch (e) {
          console.error("Durmah identity fetch failed", e);
        }
  ...
  return { upcomingTasks, todaysEvents, loading, authError, identity };
}
