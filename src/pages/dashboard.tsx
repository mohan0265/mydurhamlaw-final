      ...
      fetch("/api/dashboard/overview", {
        ...
        .then((data) => {
          ...
          if (data.upcomingAssignments?.length > 0) {
            ...
            const isAssignment =
              next.source === "assignment" ||
              (typeof next.id === "string" &&
                next.id.startsWith("assignment-"));
            const cleanId =
              typeof next.id === "string" && next.id.startsWith("assignment-")
                ? next.id.replace("assignment-", "")
                : next.id;
            ...
            const primaryLink = isAssignment
              ...
              : next.typeLabel === "Exam" || next.priorityScore === 300
                ...
                : next.yaagLink
                  ? next.yaagLink + `#event-${next.id}`
                  : "/year-at-a-glance";

            const calendarLink = next.yaagLink
              ? next.yaagLink + `#event-${next.id}`
              : "/year-at-a-glance";
            ...
            const initialFocusItem = {
              ...
              type:
                next.typeLabel ||
                (next.source === "assignment" ? "Assignment" : "Deadline"),
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-white border border-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50 transition"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-gray-900 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-gray-800 transition"
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Visit <strong>Year at a Glance</strong> to see your specific
                    term structure.
                  </p>
                  <span className="text-xs text-blue-600 font-semibold mt-2 block hover:underline">
                    Go to YAAG &rarr;
                  </span>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Upload notes or transcripts to unlock AI summaries.
                  </p>
                  <span className="text-xs text-purple-600 font-semibold mt-2 block hover:underline">
                    Go to Lectures &rarr;
                  </span>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Paste a brief to get a structure plan and research hints.
                  </p>
                  <span className="text-xs text-orange-600 font-semibold mt-2 block hover:underline">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex-1 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-black uppercase tracking-[0.1em] text-yellow-200">
                  {isFetchingOverview
              ...
              <h2 className="text-2xl md:text-3xl font-bold leading-tight">
                {isFetchingOverview ? (
                  <span className="opacity-50">Checking your tasks...</span>
                ) : dashboardError ? (
                  "Dashboard temporarily unavailable"
                ) : focusItem ? (
                  `Next deadline: ${focusItem.title}`
                ) : (
                  "You're on track"
                )}
              </h2>
              <div className="text-indigo-100 text-base md:text-lg max-w-xl">
                {isFetchingOverview ? (
                  <div className="flex items-center gap-3 animate-pulse py-4">
                    ...
                    <div className="h-4 bg-white/20 rounded-full w-48"></div>
                  </div>
                ) : dashboardError ? (
                  <div className="flex flex-col gap-4">
                    <p className="font-medium text-indigo-100 italic">
                      {dashboardError}. You can still use the Durmah buddy or
                      navigate to your Year at a Glance.
                    </p>
                    <button
                      onClick={() => window.location.reload()}
                      className="inline-flex items-center gap-2 self-start bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl text-sm font-bold border border-white/20 transition"
                    >
                      <Zap className="w-4 h-4" /> Retry Now
                    </button>
                  </div>
                ) : focusItem ? (
                    ...
                    {isMounted && focusItem.due_date && (
                      <div className="flex items-center gap-3 mt-2">
                        ...
                        <span className="text-sm font-semibold text-indigo-200 uppercase tracking-wide">
                          {(focusItem.eventDay
                            ? new Date(focusItem.eventDay)
                            : new Date(focusItem.due_date)
                  ...
                  <div className="flex flex-col gap-2">
                    <p className="font-medium text-indigo-100 mb-4 leading-relaxed">
                      No upcoming academic deadlines in the next 14 days. Great
                      work staying ahead!
                    </p>
                    <div className="flex items-center gap-3">
                      <Link
              ...
              {upcomingAssignments.length > 1 && (
                ...
                >
                  {showAllDeadlines
                    ? "Hide other deadlines"
                    : `See ${upcomingAssignments.length - 1} more deadlines in next 14 days`}
                  <ArrowRight
                    className={`w-3 h-3 transition-transform ${showAllDeadlines ? "rotate-90" : ""}`}
                  />
                </button>
              )}
                ...
                <div className="mt-4 space-y-2 animate-in fade-in slide-in-from-top-2 relative z-20">
                  {upcomingAssignments.slice(1).map((a) => {
                    const isAssignment =
                      a.source === "assignment" ||
                      (typeof a.id === "string" &&
                        a.id.startsWith("assignment-"));
                    const cleanId =
                      typeof a.id === "string" && a.id.startsWith("assignment-")
                        ? a.id.replace("assignment-", "")
                        : a.id;
                        ...
                        <div className="bg-white/10 hover:bg-white/20 border border-white/10 rounded-xl p-3 flex items-center justify-between transition backdrop-blur-sm">
                          <div className="flex items-center gap-3">
                            <div className="flex flex-col">
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] font-black text-white/70 uppercase bg-white/10 px-1.5 rounded-sm">
                                  {a.typeLabel}
                                </span>
                                <span className="text-xs text-white/60">
                                  {a.module_name || "Law Module"}
                                </span>
                              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
              <button
                onClick={() =>
                  router.push(focusItem?.yaagLink || "/year-at-a-glance")
                }
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-white text-indigo-900 font-bold px-6 py-3.5 hover:bg-indigo-50 transition shadow-lg whitespace-nowrap"
              >
              ...
              >
                ...
                <span className="absolute bottom-full mb-2 bg-black text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition whitespace-nowrap pointer-events-none">
                  Highest impact / nearest due date
                </span>
              </button>
            </div>
          </div>
              ...
              nextAssignment ? (
                <div className="mt-3">
                  <div className="flex items-center justify-between text-xs mb-1.5 gap-2">
                    ...
                    <span
                      className={`font-bold whitespace-nowrap ${nextAssignment.daysLeft <= 3 ? "text-red-600" : "text-orange-600"}`}
                    >
                      {nextAssignment.daysLeft <= 0
                        ? "Due Today"
                        : `Due ${nextAssignment.daysLeft}d`}
                    </span>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-1.5 mb-1.5">
                    <div
                      ...
                      style={{
                        width: `${Math.max(5, (nextAssignment.current_stage / 6) * 100)}%`,
                      }}
                    ></div>
                  </div>
            ...
            preview={
              <div className="mt-3 flex gap-1">
                {[1, 2, 3].map((term) => (
                  <div
                    key={term}
                    className={`flex-1 h-8 rounded border flex items-center justify-center text-[10px] font-bold ${
                      term === 1
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div
                className="bg-gradient-to-br from-purple-50 to-white rounded-2xl border border-purple-100 p-5 shadow-sm hover:shadow-md transition cursor-pointer"
                onClick={() => router.push("/study/lectures")}
              >
              ...
              <div
                className="bg-gradient-to-br from-red-50 to-white rounded-2xl border border-red-100 p-5 shadow-sm hover:shadow-md transition cursor-pointer md:col-span-2 lg:col-span-1"
              ...
              >
                ...
                <p className="text-xs text-gray-600 mb-3">
                  Key legal developments to help you connect doctrine with
                  real-world cases.
                </p>
        ...
        <div className="text-center py-8 border-t border-gray-100">
          <div className="inline-flex items-center gap-2 bg-gray-50 px-4 py-2 rounded-full border border-gray-200">
            ...
            <span className="text-xs font-medium text-gray-500">
              Built for academic integrity & Durham standards
            </span>
          </div>
        </div>
      </main>
...
function WhyThisModal({
  ...
  // Reasoning Logic
  const isExam =
    item?.reasonCodes?.includes("HIGH_PRIORITY_EXAM") ||
    item?.typeLabel === "Exam";
  const isAssignment =
    item?.reasonCodes?.includes("ASSIGNMENT_WORK") ||
  ...
  if (isExam) {
    reasonText =
      "Exams are prioritised highest because performance is time-bound and revision compounds. Early preparation allows for spaced repetition.";
    strategyText =
      "Next step: Review your revision timetable and identify one weak topic to address today.";
  } else if (isAssignment) {
    reasonText =
      "Assignments are prioritised because drafting, legal research, and referencing require significant lead time.";
    strategyText =
      "Next step: Open the assignment brief and ensure you have a valid 3-part plan (issues, authorities, structure).";
  } else if (isSoon) {
    reasonText =
      "This deadline is imminent (within 3 days). Immediate action is required to avoid penalty or stress.";
    strategyText =
      "Next step: Switch to execution mode. Focus on completion over perfection if time is tight.";
  }

  // Fallback for empty state or generic
  if (!item) {
    reasonText =
      "No immediate academic deadlines were detected in the next 14 days.";
