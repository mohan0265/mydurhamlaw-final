...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      console.log('✅ Google OAuth initiated successfully from login')
      ...
      <div className="max-w-md w-full bg-white shadow-xl rounded-lg p-6 sm:p-8">
        <div className="flex items-center justify-between mb-4 sm:mb-6 flex-wrap gap-2">
          <div className="flex items-center space-x-2 min-w-0">
            ...
            <h1 className="text-lg sm:text-xl font-bold text-gray-900 truncate"><BrandTitle variant="light" size="xl" as="span" /> Login</h1>
          </div>
          <Link href="/" className="text-sm text-purple-600 hover:underline min-h-[44px] flex items-center px-2 flex-shrink-0">Back to Home</Link>
        </div>
        ...
        <p className="mt-6 text-center text-sm text-gray-600">
          ...
          <Link href="/signup" className="text-purple-600 hover:underline font-medium min-h-[44px] inline-flex items-center px-2 -mx-2">Sign Up</Link>
        </p>
        
        {/* Debug info for development */}
        {process.env.NODE_ENV === 'development' && router.query.error && (
          <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-xs text-yellow-600">OAuth Error: {router.query.error}</p>
          </div>
        )}
      </div>
    </div>
