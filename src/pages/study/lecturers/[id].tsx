...
export default function LecturerDetail() {
  ...
  const insights = lecturer.lecturer_insights?.[0]?.insights_json || {
    ...
    pace_level: "Moderate",
    emphasis_score: 75,
    recommended_study_tactics: [
      "Review case facts before lecture",
      'Focus on "Policy" sections',
    ],
    common_pitfalls: [
      "Confusing Ratio with Obiter",
      'Missing the "Exception" cases',
    ],
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      <Head>
        <title>{lecturer.name} - Insights | Caseway</title>
      </Head>
      <GlobalHeader />
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-indigo-200 text-sm max-w-xl mt-1">
                  Learning-focused patterns from lecture transcripts — to help
                  you study smarter (not judge people).
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="p-8">
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              <InsightChip
                label="Structure"
                value={insights.structure_level || "Medium"}
                icon={<Brain className="w-4 h-4" />}
              />
              <InsightChip
                label="Pace"
                value={insights.pace_level || "Moderate"}
                icon={<Clock className="w-4 h-4" />}
              />
              <InsightChip
                label="Exam Orientation"
                value={insights.exam_orientation_level || "Medium"}
                icon={<Target className="w-4 h-4 text-red-500" />}
              />
              <InsightChip
                label="Emphasis"
                value={(insights.emphasis_score || 50) + "/100"}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
                  <CheckCircle className="w-5 h-5 text-green-600" /> Best way to
                  study for this lecturer
                </h3>
                <ul className="space-y-3">
                  {insights.recommended_study_tactics?.map(
                    (t: string, i: number) => (
                      <li
                        key={i}
                        className="flex items-start gap-2 text-gray-700 text-sm bg-green-50/50 p-3 rounded-lg border border-green-50"
                      >
              ...
              <div>
                ...
                <ul className="space-y-3">
                  {insights.common_pitfalls?.map((t: string, i: number) => (
                    <li
                      key={i}
                      className="flex items-start gap-2 text-gray-700 text-sm bg-orange-50/50 p-3 rounded-lg border border-orange-50"
                    >
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 max-w-2xl">
          ...
          <p className="text-sm text-gray-500 mb-6">
            Only visible to you. Helps refine your personal study
            recommendations.
          </p>
