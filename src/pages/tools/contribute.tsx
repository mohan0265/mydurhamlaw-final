...
function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`block w-full border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 ${props.className || ""}`}
    />
  );
}
...
function Checkbox(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      type="checkbox"
      className={`h-4 w-4 rounded border-gray-300 text-purple-600 focus:ring-purple-500 ${props.className || ""}`}
    />
  );
}
function Button(
...
export default function ContributeTool() {
  ...
  const canSubmit = useMemo(() => {
    ...
    if (form.apply_scope === "cohort" && !form.consent_affects_all)
      return false;
    return true;
  }, [isSignedIn, form]);
  ...
  async function submitNow() {
    ...
    try {
      ...
      const result = await addStudentTopic({
        ...
        day: new Date(form.date).toLocaleDateString("en-GB", {
          timeZone: "Europe/London",
          weekday: "short",
        }) as any,
        title:
          form.title ||
          `${form.kind.toUpperCase()} · ${form.module_title || safeModuleCode}`,
        notes: [
          form.notes,
      ...
      if (result?.ok) {
        ...
        setMessage({
          tone: "err",
          text: "Could not save your update right now.",
        });
      }
    } catch (e: any) {
  ...
  const preview = useMemo(
    () => ({
      ...
      title:
        form.title ||
        `${form.kind.toUpperCase()} · ${form.module_title || (form.module_code || "").toUpperCase()}`,
  ...
  return (
    <>
      <Head>
        <title>Contribute corrections • Tools • Caseway</title>
      </Head>
      ...
      <div className="min-h-screen bg-gray-50">
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <div>
              <h1 className="text-lg md:text-xl font-bold">
                Contribute a correction / update
              </h1>
              <p className="text-xs md:text-sm text-gray-600">
                Help keep your timetable & syllabus accurate.
              </p>
            </div>
            <div className="space-x-2">
              <Link href="/year-at-a-glance">
                <Button>Back to YAAG</Button>
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 grid md:grid-cols-2 gap-8">
          ...
          <form
            ...
            className="bg-white rounded-xl border p-4 md:p-6 space-y-4"
          >
            {!isSignedIn && (
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <Select
                  value={form.term}
                  onChange={(e) => updateForm("term", e.target.value as any)}
                >
              ...
              <div>
                ...
                <Select
                  value={form.kind}
                  onChange={(e) =>
                    updateForm("kind", e.target.value as EventKind)
                  }
                >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateForm("module_code", e.target.value.toUpperCase())
                  }
                />
              </div>
              <div>
            ...
            <div>
              ...
              <Textarea
                rows={4}
                placeholder="Context, source, lecturer email, etc."
                value={form.notes}
              ...
              {form.apply_scope === "cohort" && (
                <div className="mt-3 rounded-md border border-yellow-300 bg-yellow-50 p-3 text-sm text-yellow-900">
                  ...
                  <ul className="list-disc pl-5 space-y-1">
                    ...
                    <li>
                      If approved, it may update the timetable/syllabus for your
                      cohort.
                    </li>
                  </ul>
                  <label className="mt-2 inline-flex items-center gap-2">
                    <Checkbox
                      ...
                      onChange={(e) =>
                        updateForm("consent_affects_all", e.target.checked)
                      }
                    />
                    <span>
                      I understand and consent to propose this change for my
                      cohort.
                    </span>
                  </label>
                </div>
          ...
          <div className="bg-white rounded-xl border p-4 md:p-6">
            ...
            <p className="text-sm text-gray-600 mb-3">
              This is what we'll store with your submission.
            </p>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4">
          <div className="bg-white rounded-xl w-full max-w-lg p-5 border">
            <h3 className="text-lg font-semibold">
              Confirm cohort-wide submission
            </h3>
            <p className="text-sm text-gray-600 mt-2">
              You're proposing a change that may update the timetable/syllabus
              for other students once approved. Continue?
            </p>
            ...
            <div className="mt-4 flex items-center justify-end gap-2">
              ...
              <Button
                onClick={submitNow}
                variant="solid"
                disabled={submitting || !form.consent_affects_all}
              >
