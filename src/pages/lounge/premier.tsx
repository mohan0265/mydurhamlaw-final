...
export default function PremierLounge() {
  return (
    <LoungeLayout
      left={<>
        <Safe><OnlineUsers user={user} /></Safe>
        <Safe><NewFriendsCarousel /></Safe>
        <Safe><ActivityCards /></Safe>
        <Safe><MoodToggle /></Safe>
      </>}
      center={<>
        <Safe><PublicChat user={user} /></Safe>
        <Safe><LoungeFeed /></Safe>
        <Safe><Icebreakers onPick={()=>{}} /></Safe>
      </>}
      right={<>
        <Safe><ShoutoutsWall user={user} /></Safe>
        <Safe><StudyTunes /></Safe>
        <Safe><VirtualCoffeeTable /></Safe>
        <Safe><DMDrawer open={false} onClose={()=>{}} /></Safe>
      </>}
    />
  );
}