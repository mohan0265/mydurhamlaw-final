...
import { Calendar, Play, Clock, ShieldCheck } from "lucide-react";
...
export default function ExamPrepPage() {
  ...
  useEffect(() => {
    if (router.query.module) {
      ...
      const workspace = activeWorkspaces.find(
        (w) => w.module.title === mod || w.module.code === mod,
      );
  ...
  const handleSelectModuleForRevision = (
    moduleName: string,
    date: string,
    moduleId?: string,
  ) => {
  ...
  const generatePlan = () => {
    ...
    const plan = [];
    for (let i = 0; i < 4; i++) {
      // Generate 4 weeks preview
      const weekStart = addWeeks(start, i);
      plan.push(
        `Week of ${format(weekStart, "MMM d")}: ${selectedModule} - Phase ${i + 1} (Review & Practice)`,
      );
    }
      ...
      >
        <div className="p-4 sm:p-6 max-w-[1600px] mx-auto">
          ...
          {(showFamiliarityGuidance ||
            (!familiarity.exam_prep && !familiarityLoading)) && (
            <ClarityCard
              title="Strategic Exam Preparation"
              description="Review your active workspaces or build a custom revision schedule. Durmah uses your lecture notes to highlight exam-critical patterns."
              steps={[
                "Review your Active Workspaces (summaries + key cases).",
                "Generate a 4-week high-level revision plan.",
                "Chat with Durmah to create detailed daily breakdowns.",
                "Review 'Key Cases' flagged by Durmah as exam-critical.",
              ]}
              onDismiss={() => {
                markAsFamiliar("exam_prep");
                setShowFamiliarityGuidance(false);
              }}
              icon={<ShieldCheck className="w-5 h-5" />}
              watchDemoHref="/demo/exam-prep"
            />
          )}
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h2 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                  <Play className="text-violet-600" size={20} /> Active
                  Workspaces
                </h2>
                ...
                {loading ? (
                  ...
                  <div className="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                    <p className="text-sm text-gray-500 mb-2">
                      No active exam workspaces yet.
                    </p>
                    <p className="text-xs text-gray-400 mb-4">
                      Complete your lecture uploads in My Lectures to unlock.
                    </p>
                    <button
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h2 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                  <Calendar className="text-violet-600" size={20} /> Revision
                  Planner
                </h2>
                  ...
                  <div className="bg-blue-50 rounded-xl p-4 border border-blue-100 animate-in fade-in">
                    ...
                    <ul className="space-y-2">
                      {generatedPlan.map((item, i) => (
                        <li
                          key={i}
                          className="flex gap-2 text-sm text-blue-900 bg-white/50 p-2 rounded border border-blue-100"
                        >
