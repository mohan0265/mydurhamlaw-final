...
export default function YAAGCalendarPage() {
    ...
    async (date: Date) => {
      ...
      try {
        ...
        const { data: eventsData, error: eventsError } = await supabase
          .from("user_events")
          .select(
            "id, title, start_at, end_at, module_code, location, event_type, external_id",
          )
          .eq("user_id", user.id)
          .is("deleted_at", null)
  ...
  return (
    <>
      <Head>
        <title>Calendar - Caseway</title>
        <meta
          name="description"
          content="Your academic calendar with lectures and assignments"
        />
      </Head>
            ...
            <div className="mb-6 p-4 bg-gradient-to-r from-purple-100 to-indigo-100 rounded-xl border border-purple-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-purple-700 mt-1">
                  Connect your MyTimetable and Blackboard calendars to see all
                  your lectures and deadlines here.
                </p>
              </div>
              <button
