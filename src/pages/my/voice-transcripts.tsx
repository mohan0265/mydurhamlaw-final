    ...
    {path.map((segment) => (
      <div key={segment.id} className="flex items-center gap-1">
        ...
        <button
          ...
          className="hover:text-violet-600 transition font-medium text-slate-700"
        >
          {segment.name}
        </button>
      </div>
      ...
      >
        ...
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            ...
            className="p-1 hover:text-violet-600 transition"
            title="New Subfolder"
          >
...
const TranscriptDetailsModal = ({
  ...
  isOpen: boolean;
  onClose: () => void;
  transcript: VoiceJournalRow | null;
  searchQuery: string;
}) => {
  ...
  useEffect(() => {
    ...
    if (transcript.summary)
      count += (transcript.summary.match(re) || []).length;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm">
      ...
      >
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
          <div className="flex items-center gap-4">
            ...
            {searchQuery && totalMatches > 0 && (
              <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-lg p-1 shadow-sm">
                ...
                <span className="text-xs font-bold text-slate-600 px-1 min-w-[3rem] text-center">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-white rounded-xl shadow-sm transition"
          >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          {transcript.summary && (
            <div className="bg-violet-50 rounded-2xl p-5 border border-violet-100">
              <h4 className="text-[10px] font-bold text-violet-700 uppercase tracking-widest mb-2">
                Auto Summary
              </h4>
        ...
        <div className="p-6 border-t border-slate-100 flex items-center justify-between">
          <button
            onClick={() => {
              ...
              navigator.clipboard
                .writeText(text)
                .then(() => toast.success("Copied transcript"));
            }}
...
export default withAuthProtection(function VoiceTranscriptsPage() {
  ...
  const [transcriptToMove, setTranscriptToMove] =
    useState<VoiceJournalRow | null>(null);
  ...
  const handleDeleteFolder = async (id: string) => {
    if (
      !confirm(
        "Delete this folder? Mapping will be removed but transcripts remain.",
      )
    )
      return;
    try {
  ...
  const handleTogglePin = async (t: VoiceJournalRow) => {
    try {
      ...
      if (resp.ok) {
        setTranscripts((prev) =>
          prev.map((item) =>
            item.id === t.id ? { ...item, is_pinned: newVal } : item,
          ),
        );
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-[calc(100vh-64px)] bg-[#F8FAFC]">
        ...
        <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              ...
              className="p-1 hover:text-violet-600 transition text-slate-400"
          ...
          <nav className="space-y-1 mb-8">
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-semibold transition ${
                activeFolderId === "all" && !isPinnedMode
                  ? "bg-violet-50 text-violet-700"
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-semibold transition ${
                isPinnedMode && activeFolderId === "all"
                  ? "bg-violet-50 text-violet-700"
                  : "text-slate-600 hover:bg-slate-50"
              }`}
            >
              <Star
                className={`w-4 h-4 ${isPinnedMode || (activeFolderId === "all" && isPinnedMode) ? "text-amber-500 fill-amber-500" : "text-slate-400"}`}
              />
              Pinned Items
            </button>
          </nav>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  <button
                    ...
                    className={`p-2 transition ${viewMode === "grid" ? "bg-slate-100 text-violet-600" : "bg-white text-slate-400 hover:text-slate-600"}`}
                  >
                    <Grid className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => setViewMode("list")}
                    className={`p-2 transition ${viewMode === "list" ? "bg-slate-100 text-violet-600" : "bg-white text-slate-400 hover:text-slate-600"}`}
                  >
              ...
              {loading && !transcripts.length ? (
                ...
                >
                  ...
                  <p className="text-slate-500 max-w-sm mx-auto">
                    {searchQuery
                      ? "We couldn't find any transcripts matching your search."
                      : "This folder doesn't have any transcripts yet. Save a session from Durmah to fill it up!"}
                  </p>
                </motion.div>
              ) : (
                <motion.div
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                              {formatDate(t.started_at || t.created_at)}
                            </p>
                            {t.source_type === "quiz" && (
                              <div
                                className="bg-purple-100 p-0.5 rounded-md"
                                title="Quiz Session"
                              >
                                <img
                          ...
                          <h3 className="font-bold text-slate-900 leading-tight truncate group-hover:text-violet-600 transition">
                            {t.topic || "Untitled Session"}
                          </h3>
                        </div>
                        <button
      ...
      <TranscriptDetailsModal
        ...
        transcript={
          transcripts.find((t) => t.id === expandedTranscript) || null
        }
