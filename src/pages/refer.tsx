...
export default function ReferPage() {
      ...
      if (data.results) {
        ...
        data.results.forEach((r: any) => {
          ...
          const idx = newRows.findIndex(
            (row) => row.email.trim().toLowerCase() === r.email,
          );
  ...
  const getStatusBadge = (status?: string) => {
    switch (status) {
      ...
      case "already_invited":
        return <span className="text-orange-600 text-sm">Already invited</span>;
      case "invalid_domain":
        return <span className="text-red-500 text-sm">Durham email only</span>;
      case "self_referral_blocked":
        return (
          <span className="text-red-500 text-sm">Cannot invite yourself</span>
        );
      case "error":
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Head>
        <title>Refer a Friend | Caseway</title>
      </Head>
      ...
      <main className="max-w-4xl mx-auto py-12 px-4 sm:px-6">
        <div className="mb-10 text-center">
          ...
          <p className="mt-2 text-lg text-gray-600">
            Give your friends a <strong>14-day Full Access Trial</strong>. Build
            your study circle.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mb-12">
          <div className="p-6 border-b border-gray-100 bg-purple-50 flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-purple-700 mt-1">
                Invite responsibly — Durham emails only (@durham.ac.uk).
              </p>
            </div>
          ...
          <div className="p-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-center">
              <button
                onClick={addRow}
                className="text-sm font-medium text-gray-600 hover:text-purple-600 flex items-center gap-1"
              >
                  ...
                  {logs.map((log, i) => (
                    <tr key={i}>
                      ...
                      <td className="py-3 text-gray-500">
                        {new Date(log.invited_at).toLocaleDateString("en-GB", {
