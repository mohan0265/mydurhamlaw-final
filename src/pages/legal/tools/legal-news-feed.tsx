...
export default function LegalNewsFeedPage() {
  ...
  const [selectedArticle, setSelectedArticle] = useState<RSSNewsItem | null>(
    null,
  );
  ...
  // NEW: Deep Dive State
  const [deepDiveArticle, setDeepDiveArticle] = useState<RSSNewsItem | null>(
    null,
  );
...
**Important**: Analyze primarily the notes provided above, using the article metadata for context only.`;
  ...
  const fetchLiveRSSNews = useCallback(
    async (
      filter: SourceFilter = "all",
    ): Promise<{ articles: RSSNewsItem[]; metadata: any }> => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `RSS API error: ${response.status} ${response.statusText}`,
          );
        }
  ...
  const loadLiveNewsArticles = useCallback(
    async (filter: SourceFilter = sourceFilter) => {
      try {
        setLoading(true);
        setError(null);
        ...
        if (debugMode) {
          console.log(
            `🔧 Debug: Loading live RSS feeds with filter: ${filter}...`,
          );
        }

        // Fetch RSS feeds with intelligent caching
        const { articles: liveArticles, metadata } =
          await fetchLiveRSSNews(filter);
  ...
  const getEmergencyFallbackArticles = (): RSSNewsItem[] => [
    {
      ...
      summary:
        "We are experiencing temporary difficulties accessing live RSS feeds from our trusted sources. Please try refreshing in a few minutes.",
      url: "#",
      source: "Caseway System",
      sourceType: "government",
      publishedAt: new Date().toISOString(),
      topicTags: ["System", "Notice"],
      content:
        "Our live RSS aggregation service is temporarily unavailable. This may be due to network connectivity or external source maintenance.",
    },
    {
      id: "emergency-fallback-2",
      title: "Direct Access to Legal Sources",
      summary:
        "Access legal news directly from Law Gazette, UK Supreme Court, and other trusted sources.",
      ...
      content:
        "Visit trusted legal news sources directly for the most current information on UK legal developments.",
    },
  ];
  ...
  const [analysisArticle, setAnalysisArticle] = useState<RSSNewsItem | null>(
    null,
  );
  ...
  const playArticle = useCallback(
    async (article: RSSNewsItem) => {
      ...
      try {
        ...
        alert(
          `Unable to play audio: ${error instanceof Error ? error.message : "Unknown error"}. Please try again.`,
        );
      }
    },
  ...
  const filteredArticles =
    sourceFilter === "all"
      ? articles
      : articles.filter((article) => article.sourceType === sourceFilter);
          ...
          <div className="text-center py-20">
            ...
            <div className="space-y-4">
              ...
              <p className="text-lg text-gray-700 max-w-md mx-auto">
                Fetching latest updates from Law Gazette, UK Supreme Court,
                Ministry of Justice, Durham University and more...
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
        ...
        <div className="text-center mb-6 sm:mb-8 relative px-2 sm:px-0">
          ...
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-4 sm:p-6 border border-purple-200/50 shadow-lg">
            <p className="text-gray-700 text-base sm:text-lg font-medium italic text-center">
              &ldquo;Curated legal news from trusted sources, automatically
              updated twice daily at 6AM and 6PM Singapore time.&rdquo;
            </p>
            {sourceMetadata && (
              <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-6 mt-4 text-sm text-gray-600">
                ...
                {sourceMetadata.sourceBreakdown && (
                  <span className="text-center">
                    🎯{" "}
                    {sourceFilter !== "all"
                      ? `${SOURCE_FILTERS.find((f) => f.value === sourceFilter)?.label} filter`
                      : "All sources"}
                  </span>
                )}
              </div>
                ...
                <div className="flex-1">
                  ...
                  {debugMode && (
                    <div className="mt-2 p-3 bg-red-100 rounded text-xs">
                      ...
                      <br />
                      Live RSS Sources: {sourceMetadata?.feedsProcessed || 0}
                      <br />
          ...
          <Card className="mb-8 border-blue-200 bg-blue-50">
            <CardContent className="p-6">
              <h3 className="font-semibold text-blue-800 mb-3">
                🔧 Live RSS Debug Panel
              </h3>
              <div className="text-sm space-y-2">
                <div>
                  Active RSS Sources: {sourceMetadata?.feedsProcessed || 0}
                </div>
                ...
                <div>
                  UK Legal Articles:{" "}
                  {sourceMetadata?.sourceBreakdown?.ukLegal || 0}
                </div>
                <div>
                  Government Articles:{" "}
                  {sourceMetadata?.sourceBreakdown?.government || 0}
                </div>
                <div>
                  Current Filter:{" "}
                  {SOURCE_FILTERS.find((f) => f.value === sourceFilter)?.label}
                    ...
                    onClick={() => {
                      ...
                      fetchLiveRSSNews(sourceFilter)
                        .then(({ articles, metadata }) =>
                          console.log("✅ Live RSS Test Success:", {
                            articles: articles.length,
                            metadata,
                          }),
                        )
                        .catch((error) =>
                          console.error("❌ Live RSS Test Failed:", error),
                        );
                    }}
                  ...
                  <button
                    onClick={() => {
                      console.log("📊 Articles by Source:", {
                        durham: articles
                          .filter((a) => a.sourceType === "durham")
                          .map((a) => ({ title: a.title, source: a.source })),
                        ukLegal: articles
                          .filter((a) => a.sourceType === "uk-legal")
                          .map((a) => ({ title: a.title, source: a.source })),
                        government: articles
                          .filter((a) => a.sourceType === "government")
                          .map((a) => ({ title: a.title, source: a.source })),
                      });
                    }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <CardContent className="p-4 sm:p-6">
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 pt-4 border-t border-gray-100">
                    ...
                    <Button
                      ...
                      disabled={
                        loading ||
                        (!voiceManager.canPlayAudio("news") &&
              ...
              <Card>
                <CardContent className="p-6 sm:p-12 text-center">
                  ...
                  <p className="text-sm sm:text-base text-gray-500">
                    No articles available for the selected filter. Try changing
                    the source filter or refreshing the feeds.
                  </p>
                </CardContent>
              </Card>
            )}
          ...
          <div className="lg:col-span-1">
            <div
              className="sticky top-4 sm:top-8 max-h-[60vh] sm:max-h-[70vh] lg:max-h-[75vh] xl:max-h-[80vh] overflow-y-auto pr-2 scroll-smooth smart-analysis-panel"
                ...
                <Card>
                  <CardContent className="p-6 sm:p-8 text-center">
                    ...
                    <p className="text-gray-500 text-sm leading-relaxed">
                      Click &ldquo;Get AI Analysis&rdquo; on any article to
                      receive intelligent insights about its relevance to your
                      law studies, discussion questions, and connections to
                      legal modules.
                    </p>
                  </CardContent>
                </Card>
