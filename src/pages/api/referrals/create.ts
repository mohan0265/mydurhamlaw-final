...
export default async function handler(
  ...
  if ((count || 0) + invites.length > 10) {
    return res
      .status(429)
      .json({ error: "Daily invite limit reached (10/day)." });
  }
      ...
      if (resend) {
        try {
          await resend.emails.send({
            from: "Caseway <invites@casewaylaw.ai>",
            to: email,
            replyTo: "support@casewaylaw.ai",
            subject: "You've been invited to Caseway",
            html: `
                        <div style="font-family: sans-serif; padding: 20px;">
                            <h2>You've been invited!</h2>
                            <p>${user.user_metadata?.first_name || "A fellow law student"} has invited you to join Caseway.</p>
                            <p><strong>Strictly for law students.</strong></p>
                            <p>Get a <strong>14-day Pro Trial</strong> (access to Durmah Voice, Lecture Signals, and more) when you join securely.</p>
                            <br/>
                            <a href="${process.env.NEXT_PUBLIC_SITE_URL || "https://casewaylaw.ai"}/redeem?token=${token}" style="background-color: #7e22ce; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">
                                Accept Invite & Start Trial
                            </a>
                            <br/><br/>
                            <p style="font-size: 12px; color: #888;">
                                <a href="${process.env.NEXT_PUBLIC_SITE_URL || "https://casewaylaw.ai"}/unsubscribe">Stop receiving invites</a>
                            </p>
                        </div>
                    `,
          });
