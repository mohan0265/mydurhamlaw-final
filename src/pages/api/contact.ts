...
export default async function handler(
  ...
  try {
    ...
    const safeSubject = (subject || "New enquiry from Caseway").slice(0, 200);
    ...
    const html = `
      <div style="font-family: system-ui, -apple-system, Segoe UI, Roboto, sans-serif; line-height:1.6; max-width:640px; margin:auto">
        <div style="background:linear-gradient(90deg,#6d28d9,#4338ca); color:#fff; padding:18px 20px; border-radius:10px 10px 0 0">
          <h1 style="margin:0; font-size:20px;">Caseway - New Contact Form Message</h1>
        </div>
        ...
        <div style="color:#6b7280; font-size:12px; text-align:center; margin-top:10px">
          Sent from the contact form on casewaylaw.ai
        </div>
      </div>
    `;
    ...
    try {
      const mod = await import("@/lib/server/supabaseAdmin").catch(
        () => null as any,
      );
    ...
    return res
      .status(200)
      .json({ success: true, id: (result as any)?.data?.id || null });
  } catch (err: any) {
