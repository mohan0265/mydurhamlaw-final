...
import type {
  CreateTestLovedOneRequest,
  CreateTestLovedOneResponse,
} from "@/types/admin";
...
export default async function handler(
  ...
  const {
    ...
    password,
    isTest,
  }: CreateTestLovedOneRequest = req.body;
  ...
  try {
    ...
    const { data: existingUserId, error: rpcError } = await adminClient.rpc(
    ...
    if (existingUserId) {
      ...
      console.log(
        `[create-loved-one] No existing user, creating new account for ${email}`,
      );
      ...
      if (authError || !authData?.user) {
        console.error(
          "[create-loved-one] createUser failed:",
          authError?.message,
        );
        return res
          .status(400)
    ...
    if (existingConnection) {
      console.log(
        `[create-loved-one] Connection already exists: ${existingConnection.id}`,
      );
      connectionData = existingConnection;

      // Ensure status is active/granted
      if (
        existingConnection.status !== "granted" &&
