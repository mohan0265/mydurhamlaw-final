  ...
  try {
    ...
    if (!forceFresh) {
      ...
      if (cachedArticles.length > 0) {
        console.log(
          `✅ Serving ${cachedArticles.length} articles from intelligent cache`,
        );
        ...
        return res.status(200).json({
          ...
          sourceBreakdown: {
            durham: cachedArticles.filter((a) => a.sourceType === "durham")
              .length,
            ukLegal: cachedArticles.filter((a) => a.sourceType === "uk-legal")
              .length,
            government: cachedArticles.filter(
              (a) => a.sourceType === "government",
            ).length,
          },
    ...
    const parser = new Parser({
      ...
      headers: {
        "User-Agent": "Caseway-RSS-Reader/2.0 (Smart-Cache)",
      },
    });
    ...
    const feedPromises = enabledFeeds.map(async (feedConfig) => {
      try {
        ...
        if (feed.items && feed.items.length > 0) {
          feed.items.slice(0, 20).forEach((item, index) => {
            ...
            const article: RSSArticle = {
              ...
              topicTags: [
                feedConfig.category,
                ...extractAdvancedTopicTags(
                  title,
                  cleanContent,
                  feedConfig.sourceType,
                ),
              ],
        ...
        console.log(
          `✅ Successfully parsed ${articles.length} articles from ${feedConfig.name}`,
        );
        return articles;
      } catch (error) {
        console.error(
          `❌ Failed to fetch ${feedConfig.name}:`,
        ...
        if (cached && Date.now() - cached.timestamp < 2 * 60 * 60 * 1000) {
          // 2 hours fallback
          console.log(
            `🔄 Using stale cache for ${feedConfig.name} due to fetch error`,
          );
          return cached.data;
        }
    ...
    feedResults.forEach((result, index) => {
      if (result.status === "fulfilled") {
        ...
        console.warn(
          `⚠️ Feed ${enabledFeeds[index]?.name || "unknown"} failed:`,
          result.reason,
        );
      }
    });
    ...
    return res.status(200).json({
      ...
      sourceBreakdown: {
        ...
        government: allArticles.filter((a) => a.sourceType === "government")
          .length,
      },
      timestamp: new Date().toISOString(),
    ...
    try {
      ...
      if (emergencyCache.length > 0) {
        console.log(
          `🚨 Using emergency cache: ${emergencyCache.length} articles`,
        );
        return res.status(200).json({
          success: true,
          ...
          totalSources: RSS_FEEDS.length,
          sourceBreakdown: {
            durham: emergencyCache.filter((a) => a.sourceType === "durham")
              .length,
            ukLegal: emergencyCache.filter((a) => a.sourceType === "uk-legal")
              .length,
            government: emergencyCache.filter(
              (a) => a.sourceType === "government",
            ).length,
          },
          timestamp: new Date().toISOString(),
 ...
 */
function extractAdvancedTopicTags(
  title: string,
  content: string,
  sourceType: string,
): string[] {
