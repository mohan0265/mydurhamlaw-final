...
export default async function handler(
  ...
  try {
    ...
    if (!resend) {
      console.warn(
        "LNAT Waitlist: Resend not configured. Logging submission:",
        { name, email, role, year },
      );
      return res
        .status(200)
        .json({ ok: true, message: "Waitlist entry recorded (Mock Mode)" });
    }

    await resend.emails.send({
      from: "Caseway <noreply@casewaylaw.ai>",
      to: "support@casewaylaw.ai",
