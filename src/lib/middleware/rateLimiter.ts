// Enhanced Rate Limiter Middleware for MyDurhamLaw API Protection
...
// Pre-configured rate limiters for different MyDurhamLaw endpoints

// Strict limits for authentication endpoints
export const authRateLimit = createRateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  maxRequests: 5, // Only 5 auth attempts per 15 minutes
  message: 'Too many authentication attempts. Please try again later for security.',
  statusCode: 429
})
...
export const voiceRateLimit = createRateLimit({
  ...
  message: 'Voice processing limit exceeded. Please wait before using voice features again.',
  statusCode: 429
})
...
export function createUserRateLimit(options: RateLimitOptions & { getUserId?: (req: NextApiRequest) => string | null } = {}) {
