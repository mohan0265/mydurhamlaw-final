...
class RSSScheduler {
  ...
  private async hasRSSContentChanged(
    ...
    try {
      const headers: Record<string, string> = {
        "User-Agent": "Caseway-RSS-Reader/2.0",
      ...
      console.error(
        `Failed to check RSS content change for ${sourceUrl}:`,
        error,
      );
   ...
   */
  private async updateRSSCache(
    cacheEntry: Partial<RSSCacheEntry>,
  ): Promise<void> {
    const supabase = getSupabaseClient();
    if (!supabase) return;
  ...
  private async logScheduleExecution(
    logEntry: Omit<RSSScheduleLog, "id">,
  ): Promise<void> {
    const supabase = getSupabaseClient();
    if (!supabase) return;
  ...
  async fetchRSSWithIntelligentCaching(): Promise<{
    ...
    if (this.isRunning) {
      this.logger.warn(
        "scheduler",
        "RSS fetch already running, skipping duplicate request",
      );
      return {
        success: false,
        articlesProcessed: 0,
    ...
    try {
      ...
      this.logger.info(
        "cache",
        `Found ${cachedData.length} cached RSS sources`,
      );
      ...
      for (const feed of RSS_FEEDS) {
        try {
          const cachedEntry = cachedData.find(
            (entry) => entry.source_url === feed.url,
          );
          ...
          if (!contentCheck.changed && cachedEntry) {
            this.logger.info(
              feed.name,
              "Skipping - no content changes detected",
              {
          ...
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/api/rss-news?source=${encodeURIComponent(feed.name)}`,
            {
              method: "GET",
              headers: { "User-Agent": "Caseway-Scheduler/2.0" },
            },
          );
          ...
          await this.updateRSSCache({
            id:
              cachedEntry?.id ||
              `${feed.name.toLowerCase().replace(/\s+/g, "-")}-${Date.now()}`,
          ...
          if (cachedEntry) {
            try {
              await this.updateRSSCache({
                ...
                status:
                  (cachedEntry.error_count || 0) >= 2 ? "error" : "active",
                metadata: {
              ...
              console.error(
                "Failed to update error count in cache:",
                cacheError,
              );
            }
          }
      ...
      this.logger.error("scheduler", "Critical error in RSS fetch", {
        error: errorMessage,
      });
