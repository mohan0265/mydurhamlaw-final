// API Security utilities for Caseway
...
export function validateEnvironmentSecurity() {
  ...
  if (clientKeys.length > 0) {
    throw new Error(
      `SECURITY VIOLATION: Sensitive keys exposed to client: ${clientKeys.join(", ")}`,
    );
  }
}
...
export function validateOpenAIKeySecurity() {
  ...
  const clientSideKeys = Object.keys(process.env).filter(
    (key) => key.startsWith("NEXT_PUBLIC_") && key.includes("OPENAI"),
  );
...
export async function authenticateAPIUser(
  ...
  try {
    ...
    const sessionToken =
      authHeader?.replace("Bearer ", "") || req.cookies["sb-access-token"];
...
export function validateClientIP(req: NextApiRequest): {
  ip: string;
  isValid: boolean;
